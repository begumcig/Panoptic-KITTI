...
class PanopticDeepLabTargetGenerator(object):
    ...
    def __call__(self, panoptic, segments_info, prev_panoptic=None):
        ...
        frame_offset = np.zeros((2, height, width), dtype=np.float32)
        ...
        frame_offset_weights = np.zeros_like(panoptic, dtype=np.uint8)

        if prev_panoptic is not None:
            (prev_center, prev_unique_ids, prev_centers_x, prev_centers_y) = self._generate_prev_centers_with_noise(prev_panoptic)
        ...
        for seg in segments_info:
            ...
            if not seg["iscrowd"]:
                ...
                if not self.ignore_stuff_in_offset or cat_id in self.thing_ids:
                    ...
                    frame_offset_weights[panoptic == seg["id"]] = 1
            if cat_id in self.thing_ids and not seg["iscrowd"]: ####added 
                ...
                if prev_panoptic is not None:
                    prev_mask = prev_unique_ids == seg["id"]
                    if np.count_nonzero(prev_mask) > 0:
                        prev_center_x = prev_centers_x[prev_mask]
                        prev_center_y = prev_centers_y[prev_mask]

                        frame_offset[0][mask_index] = prev_center_y - y_coord[mask_index]
                        frame_offset[1][mask_index] = prev_center_x - x_coord[mask_index]
        ...
        frame_offset_weights = frame_offset_weights[None]
        return dict(
            sem_seg=torch.as_tensor(semantic.astype("long")),
            center=torch.as_tensor(center.astype(np.float32)),
            prev_center = torch.as_tensor(prev_center.astype(np.float32)),
            center_points=center_pts,
            offset=torch.as_tensor(offset.astype(np.float32)),
            frame_offset = torch.as_tensor(frame_offset.astype(np.float32)),
            ...
            frame_offset_weights=torch.as_tensor(frame_offset_weights.astype(np.float32)),
        )

    def _generate_prev_centers_with_noise(self, panoptic, offset_noise_factor = 0.05, false_positive_rate=0.5, false_positive_noise_factor = 0.05):
        height, width = panoptic.shape[0], panoptic.shape[1]
        center = np.zeros((height, width), dtype=np.float32)
        centerf = np.zeros((height, width), dtype=np.float32)
        unique_ids = np.unique(panoptic)

        ids_to_center_x = np.zeros_like(unique_ids, dtype=np.int32)
        ids_to_center_y = np.zeros_like(unique_ids, dtype=np.int32)


        for pan_id in unique_ids:
            semantic_id = pan_id // self.label_divisor
            if(semantic_id == self.ignore_label or pan_id % self.label_divisor == 0):
                continue
            mask_index = np.where(panoptic == pan_id)
            ...
            center_y, center_x = np.mean(mask_index[0]), np.mean(mask_index[1])
            max_y, max_x = np.max(mask_index[0]), np.max(mask_index[1])
            min_y, min_x = np.min(mask_index[0]), np.min(mask_index[1])
            bbox_y, bbox_x = max_y - min_y , max_x - min_x

            rng= np.random.default_rng()

            center_y = (center_y + rng.standard_normal(dtype=np.float32) * offset_noise_factor * bbox_y )
            center_x = (center_x + rng.standard_normal(dtype=np.float32) * offset_noise_factor * bbox_x )

            center_y = min(max(np.round(center_y).astype(np.float32), 0), height - 1)
            center_x = min(max(np.round(center_x).astype(np.float32), 0), width - 1)

            id_index  = np.where(np.equal(pan_id, unique_ids))
            ids_to_center_x[id_index] = center_x
            ids_to_center_y[id_index] = center_y
            ...
            center_y = (center_y + rng.standard_normal(dtype=np.float32) * false_positive_noise_factor * bbox_y )
            center_x = (center_x + rng.standard_normal(dtype=np.float32) * false_positive_noise_factor * bbox_x )
            ...
            center_x0, center_x1 = max(0, ul[0]), min(br[0], width)
            center_y0, center_y1 = max(0, ul[1]), min(br[1], height)
            centerf[center_y0:center_y1, center_x0:center_x1] = np.maximum(
                center[center_y0:center_y1, center_x0:center_x1],
                self.g[gaussian_y0:gaussian_y1, gaussian_x0:gaussian_x1],
            )
            

            center = center + rng.standard_normal(dtype=np.float32) < false_positive_rate * (centerf- center)

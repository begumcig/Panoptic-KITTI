...
class PanopticDeeplabDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        #prev_pan_seg_gt = utils.read_image(dataset_dict["prev_pan_seg_file_name"], "RGB")
        ...
        #dataset_dict["prev_pan_image"] = torch.as_tensor(np.ascontiguousarray(prev_pan_seg_gt.transpose(2, 0, 1)))
        
        prev_pan_seg_gt = np.array(Image.open(dataset_dict["prev_pan_seg_file_name"]), dtype=np.uint32)
        prev_pan_seg_gt = prev_pan_seg_gt[:,:,0] * 1000 + prev_pan_seg_gt[:,:,1] * 256 + prev_pan_seg_gt[:,:,2] 
        # Generates training targets for Panoptic-DeepLab.
        targets = self.panoptic_target_generator(rgb2id(pan_seg_gt), dataset_dict["segments_info"], prev_panoptic=prev_pan_seg_gt)
        dataset_dict.update(targets)

        return dataset_dict

class PanopticDeeplabTestMapper:
    @configurable
    def __init__(
        self,
        *,
